...
public class EggSpawner : MonoBehaviour
{
    ...
    void PlaceAllEggs()
    {
        ...
        GameObject goldenEgg = Instantiate(goldenEggPrefab, goldenEggSpawnContainer.transform.GetChild(randomNum).position, goldenEggSpawnContainer.transform.GetChild(randomNum).rotation) as GameObject;
        
        
        // spawns a certain number of eggs
        for (int i = 0; i < numOfEggs; i++)
        {
            GameObject egg = SpawnRandomEgg();

            PositionEgg(egg);
        }
    }

    GameObject SpawnRandomEgg()
    {
        ...
        if (random == 0)
        {
            egg = Instantiate(eggPrefab, randomPosition, Quaternion.identity) as GameObject;
        }
        else
        {
            egg = Instantiate(egg2Prefab, randomPosition, Quaternion.identity) as GameObject;
        }

        return egg;
    }

    void PositionEgg(GameObject egg)
    {
        RaycastHit hit;
